...
> **تحديث 2026-09-13 (المهمة 12 — تحصين الإنتاج):** المصادقة الفعلية صارت **TOTP بدون SMS** (RFC 6238 عبر تطبيقات المصادقة + رموز استرداد + رموز تفعيل إدارية R+7) — قناة SMS-OTP في عقود §4 أدناه **مُستبدلة** بعقد التنفيذ الفعلي §4.0، وأُضيفت عقود التنفيذ المنفَّذة: المصادقة (§4.0)، رفع KYC الفعلي multipart (§5)، الأحداث الأمنية وإعادة تعيين المصادقة للوحة (§22)، وأكواد أخطاء التنفيذ القصيرة (§2.1). كذلك: `typescript.ignoreBuildErrors` أُزيل (البناء صارم الأنواع وتُنفّذه CI)، وقيود IP الإدارية عبر `ADMIN_IP_ALLOWLIST` (RBAC-002) مفروضة في وسيط مسارات `/api/admin/*`.
...
> **تحديث 2026-09-13 (المهمة 12):** التنفيذ الفعلي (خادم Next.js الحالي) يستخدم عائلة أكواد قصيرة مضمنة في `src/lib/api-types.ts` (`ERROR_MESSAGES`) بنفس دلالات العائلات أعلاه (AUTH-002/003 ≈ ERR_AUTH_OTP_INVALID/MAX_ATTEMPTS؛ AUTH-001 ≈ ERR_AUTH_SESSION_EXPIRED؛ PIN-001/002 ≈ ERR_AUTH_PIN_INCORRECT/LOCKED). **أُزيلت** `AUTH-004` (مهلة إعادة إرسال SMS-OTP) و`AUTH-006` (حجب الدخول التجريبي) مع زوال القناتين نهائياً، و**أُضيف**:

| الكود (التنفيذ) | HTTP | رسالة المستخدم | المعنى التقني | يقابل في التصميم |
|---|---|---|---|---|
| AUTH-007 | 403 | المصادقة غير مفعّلة لهذا الحساب — استخدم رمز التفعيل الصادر من الدعم | `totpConfirmedAt` فارغ — يلزم رمز تفعيل R+7 (A5) | بند جديد مع TOTP |
| AUTH-901 | 409 | هذا الرقم مسجّل مسبقاً — سجّل الدخول بدلاً من إنشاء حساب جديد | تسجيل على رقم مؤكد المصادقة مسبقاً | ERR_AUTH_PHONE_EXISTS |
| AUTH-902 | 409 | الحساب مفعّل المصادقة بالفعل — سجّل الدخول برمز المصادقة | تأكيد تسجيل لحساب مفعّل TOTP | ERR_AUTH_PHONE_EXISTS |
| RBAC-002 | 403 | الوصول الإداري مقيّد بعناوين شبكة معتمدة | IP خارج قائمة `ADMIN_IP_ALLOWLIST` (وسيط `/api/admin/*`) | قيود IP الإدارية (FR-ADM-002) |

> **تحديث 2026-09-14 (المهمة 14 — إحكام نزاهة النواة المالية):**
> 1. **`TXN-004` (409) جديد**: طلبان متزامنان بنفس `Idempotency-Key` — الأول يفوز وينفّذ، والثاني يصطدم بقيد الفريد فيعاد له هذا الكود القابل لإعادة المحاولة (`retryAfterSec`) بدل خطأ عام 500؛ إعادة المحاولة تمر بالفحص المسبق فتعيد نتيجة العملية الأصلية (`replayed=true`) — عقد AC-03 يبقى محفوظاً حتى تحت السباقات.
> 2. **`GET /api/admin/ledger-check` موسّع (M14)**: إضافة مطابقة كاملة للأرصدة — الاستجابة صارت تشمل `balancesReconciled` / `balanceViolations[]` / `walletsChecked` / `walletsWithoutEntries` إلى جانب `ledgerBalanced` (Σ=0). الفحص يعيد بناء سلسلة رصيد كل محفظة من قيودها (`balanceAfterMinor`) ويتحقق أن الرصيد المخزن الحالي يطابق نهاية السلسلة بالضبط — أي تلاعب أو تلف في `Wallet.balanceMinor` أو في أي قيد يُكشف فوراً.
> 3. **حدود يومية شاملة**: أنواع `TOPUP`/`BILL_PAY`/`CARD_PURCHASE`/`MERCHANT_PAY_OUT` صارت تُحصى ضمن الحدود اليومية (عدد + مبلغ) كما كانت التحويلات والحوالات — كانت تُفحص نظرياً دون أن تتراكم.
> 4. **سبب الإجراء الإداري يُحفظ**: مسارات `PUT fees/limits/fx/services` تقرأ الآن `reason` من الجسم وتكتبه في `AuditLog` (كان الحقل يُرسل من الواجهة ويُهمَل خادمياً).
> 5. **`clientIp` من آخر قفزة XFF**: `rate-limit.ts` والوسيط يأخذان آخر قيمة في `X-Forwarded-For` (التي يضيفها Caddy الموثوق) بدل الأولى القابلة للتزوير من العميل — يغلق تجاوز حدود المعدل و`ADMIN_IP_ALLOWLIST` بتزوير IP.
...
> **تحديث 2026-09-13 (المهمة 12) — قرار إنتاجي مُلزم:** قناة SMS-OTP (البنود EP-AUTH-01/02/04/05 أدناه) **مُستبدلة بالتنفيذ الفعلي** بمصادقة **TOTP بدون SMS** — الجدول أدناه يبقى مرجعاً تصميمياً لتعدد القنوات إن فُعّلت لاحقاً فقط. **عقد التنفيذ الملزم فعلاً هو §4.0.**

## 4.0 عقد التنفيذ الفعلي — المصادقة TOTP (منفَّذ، إلزامي)

> المصدر: `src/app/api/auth/**` + `src/lib/server/totp.ts` / `totp-service.ts` / `crypto-vault.ts` (تحقّق من الكود عند أي تعديل). الجلسة: كوكي `sw_session` httpOnly صلاحية 7 أيام + قناة احتياطية **ترويسة `x-sw-session`** (iframe-safe) بانتهاء خادمي موحّد على القناتين.

| ID | Method | المسار | الغرض | التوثيق | الدور/الصلاحية | المرحلة |
|---|---|---|---|---|---|---|
| A1 | POST | /api/auth/login `{ phone }` | بدء الدخول: يعيد `{ exists, enrolled, frozen }` لتوجيه الواجهة (رمز 6 خانات أو رمز تفعيل). حدود: 30 طلب/ساعة لكل IP + 10/15 دقيقة لكل هاتف (SYS-002) | عام | زائر | منفَّذ |
| A2 | POST | /api/auth/login/verify `{ phone, code }` | تحقق الدخول بثلاث صيغ رمز: TOTP 6 خانات (نافذة ±30 ث + منع إعادة الاستخدام) / رمز استرداد `XXXX-XXXX` لمرة واحدة / رمز تفعيل `R+7` → فرع `reEnroll` يعرض إلحاقاً جديداً. النجاح → `AuthResultView` (user + needsPin + notice + sessionToken + recoveryRemaining) + جلسة. فشل: 5 محاولات/15 دقيقة لكل هاتف (AUTH-003) | عام | زائر | منفَّذ |
| A3 | POST | /api/auth/register `{ phone, fullName, governorate }` | إنشاء حساب (ACTIVE بلا أموال + المحافظ الثلاث + إشعار ترحيب) + إصدار إلحاق TOTP **مرة واحدة**: `{ enrollment: { secret, otpauthUrl, qrDataUrl } }` (مسح QR + إدخال يدوي). AUTH-901 لو الرقم مؤكد المصادقة مسبقاً | عام | زائر | منفَّذ |
| A4 | POST | /api/auth/register/confirm `{ phone, code }` | تأكيد الإلحاق برمز التطبيق → تفعيل TOTP + **8 رموز استرداد تُعرض مرة واحدة** + جلسة (`AuthResultView.recoveryCodes`). AUTH-902 لو مفعّل مسبقاً، AUTH-007 لو لا إلحاق صادراً | عام | زائر | منفَّذ |
| A5 | POST | /api/auth/login/reenroll-confirm `{ phone, token, code }` | إتمام إعادة الإلحاق بعد إعادة تعيين إدارية: رمز `R+7` (يُستهلك هنا — 24 ساعة، لمرة واحدة) + رمز TOTP صحيح للسر الجديد → تفعيل + جلسة + رموز استرداد جديدة | عام | زائر | منفَّذ |
| A6 | POST | /api/auth/recovery/regen `{ code }` | إعادة توليد رموز الاسترداد (يتطلب رمز TOTP حالياً صحيحاً) → 8 رموز جديدة تُعرض مرة واحدة + حدث RECOVERY_REGEN. حد: 3/ساعة لكل مستخدم | (جلسة) | كل المستخدمين الموثّقين | منفَّذ |
| A7 | POST | /api/auth/logout | إبطال الجلسة الحالية | (جلسة) | عميل | منفَّذ |

ملاحظات عقد §4.0:

- **الأسرار:** سر TOTP لكل مستخدم (20 بايتاً بصيغة Base32) **مشفّر AES-256-GCM بمفتاح `APP_KEY`** (متغير بيئة ≥ 32 محرفاً — إلزامي في الإنتاج، fail-closed؛ توليده تلقائي داخل الحاوية عند غيابه) — سرقة ملف القاعدة وحده لا تكفي لاستنساخ المصادقة (`src/lib/server/crypto-vault.ts`).
- **الحماية:** قفل فشل لكل هاتف 5 محاولات/15 دقيقة (AUTH-003) فوق حدود IP (30 محاولة/ساعة) + **منع إعادة استخدام الرمز** (تخزين الخطوة الزمنية الناجحة `totpLastStep`) + أحداث أمنية لكل فشل/قفل/نجاح (§22).
- **رمز التفعيل `R+7`:** يصدره الدعم حصراً عبر الإجراء الإداري EP-ADM-RESET (§22) بعد التحقق من هوية المستخدم — صلاحية 24 ساعة ولمرة واحدة.
- **إجراءات فقدان الجهاز:** رمز استرداد واحد يكفي للدخول؛ إن فُقدت الرموز كلها → إعادة تعيين إدارية (TOTP_RESET) ثم إلحاق جديد.
- **حُذفت نهائياً من الكود:** `POST /api/auth/otp` و`POST /api/auth/verify` (SMS-OTP) و`POST /api/auth/demo-login` (الدخول التجريبي) — لا قنوات تطوير في الإنتاج (Master §140).

**عقد التصميم المستهدف الأصلي (EP-AUTH — قناة SMS-OTP، مُستبدل بالتنفيذ أعلاه):**
...
> **تحديث 2026-09-13 (المهمة 12-g):** التنفيذ الفعلي (MVP الحالي) يستخدم **رفعاً مباشراً multipart عبر الـAPI نفسه** (لا Signed URLs بعد — يبقى ADR-008 الهدف عند اعتماد تخزين S3-compatible)، مع فحص **البايتات السحرية** للنوع (لا ثقة بأي Content-Type من العميل) وحفظ الملفات بأسماء عشوائية داخل `uploads/kyc/{userId}/` بصلاحية 0600 — والعرض عبر مسار محمي بتفويض لا روابط موقعة:

| ID | Method | المسار | الغرض | التوثيق | الدور/الصلاحية | المرحلة |
|---|---|---|---|---|---|---|
| K2i | POST | /api/kyc (multipart/form-data) | تقديم الطلب: حقول نصية (fullName, idType, idNumber, governorate, address?, occupation?, monthlyIncomeMinor?) + **docFile إلزامي** (JPEG/PNG/WEBP حتى 5MB) + selfieFile? اختياري → PENDING + إشعار. KYC-002 لو طلب قائم؛ إعادة الإرسال بعد REJECTED تحذف مرفوعات التقديم السابق | (جلسة) | عميل | منفَّذ |
| K3i | GET | /api/kyc/file?userId&kind=doc\|selfie | جلب مستند مرفوع لعرضه/معاينته: المالك فقط أو ADMIN/COMPLIANCE لأي مستخدم (RBAC-001) — اسم الملف من القاعدة حصراً (منع اجتياز مسار مزدوج) + `Cache-Control: private, no-store` | (جلسة) | مالك / ADMIN / COMPLIANCE | منفَّذ |
...
> **تحديث 2026-09-13 (المهمة 12):** أُضيف للتنفيذ الفعلي قسم أمني للوحة الإدارة (الأحداث الأمنية + إعادة تعيين مصادقة المستخدم) — وقيدت القنوات الإدارية IP عبر وسيط `src/middleware.ts` يفرض `ADMIN_IP_ALLOWLIST` على كل `/api/admin/*` (اختياري: بلا ضبط للمتغير لا تقييد — والمفروض حالياً TOTP قوي + جلسات قابلة للإبطال):

| ID | Method | المسار | الغرض | التوثيق | الصلاحية المطلوبة | المرحلة |
|---|---|---|---|---|---|---|
| EP-ADM-EVT | GET | /api/admin/security-events?cursor&kind | سجل الأحداث الأمنية (LOGIN_FAIL/LOGIN_LOCK/LOGIN_SUCCESS، REGISTER، TOTP_RESET، RECOVERY_USED/REGEN، REENROLL_ISSUED/USED، ADMIN_IP_DENIED) أحدثها أولاً بترقيم cursor وفلتر اختياري بالنوع | (جلسة) | ADMIN / COMPLIANCE | منفَّذ |
| EP-ADM-RESET | POST | /api/admin/users/{id}/reset-totp `{ reason }` | إعادة تعيين المصادقة بعد التحقق من هوية المستخدم عبر الدعم: إبطال كل جلساته + مسح سر TOTP ورموز الاسترداد + إصدار رمز تفعيل R+7 صالح 24 ساعة لمرة واحدة (يُسلّم عبر قناة موثوقة) + رفع قفل الهاتف + Audit (TOTP_RESET) + إشعار + حدثان أمنيان | (جلسة) | ADMIN | منفَّذ |
