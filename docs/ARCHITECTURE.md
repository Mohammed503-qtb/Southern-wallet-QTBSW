...
> **تحديث 2026-09-13 (المهمة 12 — تحصين الإنتاج):** أُضيفت لـ§8 (الأمان المعماري) طبقة **المصادقة الثنائية TOTP المنفَّذة فعلياً** (§8.1 صف «المصادقة الثنائية TOTP» + §8.3 تدفق الأحداث الأمنية): أسرار مشفرة AES-256-GCM بمفتاح **APP_KEY** خارج القاعدة (`crypto-vault.ts`)، منع إعادة استخدام الرمز (totpLastStep)، قفل محاولات لكل هاتف، أحداث أمنية دائمة (SecurityEvent)، وبوابة IP إدارية اختيارية في وسيط `/api/admin/*` (RBAC-002). يبقى الجدول أدناه مرجعاً تصميمياً للمستهدف الكامل (JWT/Rotation/KMS/Redis) — والانحراف المنفَّذ حالياً: الجلسات رموز خادمية 7 أيام بدل JWT، وscrypt بدل Argon2id (كلاهما موثق في docs/PRODUCTION_READINESS.md).
...
| المصادقة الثنائية (TOTP) | **منفَّذة (2026-09-13) — TOTP بلا SMS خادمي حصراً** (RFC 6238): سر 20 بايتاً لكل مستخدم **مشفّر AES-256-GCM بمفتاح APP_KEY خارج قاعدة البيانات** (`crypto-vault.ts` — fail-closed في الإنتاج)، نافذة ±30 ث، **منع إعادة استخدام الرمز** (تخزين الخطوة الناجحة totpLastStep)، 8 رموز استرداد ملبَّدة scrypt لمرة واحدة، رموز تفعيل إدارية R+7 (24 ساعة)، قفل 5 محاولات/15 دقيقة لكل هاتف | FR-AUTH-001/002/013، ADR-006، FR-ADM-002 |
...
| MFA للإدارة | TOTP إلزامي لكل أدوار اللوحة + قيود IP اختيارية — **منفَّذ (2026-09-13)**: لا قناة دخول إلا TOTP لكل الأدوار + `ADMIN_IP_ALLOWLIST` في وسيط `/api/admin/*` | FR-ADM-002، Master PLAN §78 |
| أحداث الأمن (SecurityEvent) | **منفَّذة (2026-09-13)**: سجل خادمي دائم (LOGIN_FAIL/LOGIN_LOCK/LOGIN_SUCCESS، REGISTER، TOTP_RESET، RECOVERY_USED/REGEN، REENROLL_ISSUED/USED، ADMIN_IP_DENIED) + لوحة إدارية (GET /api/admin/security-events) — كتابة best-effort لا تُفشل العملية (§8.3) | Master §83، FR-ADM-051، NFR-MNT |
| بوابة IP الإدارية | وسيط `src/middleware.ts` يفرض ADMIN_IP_ALLOWLIST على كل `/api/admin/*`: IP خارج القائمة → 403 RBAC-002 + تسجيل — بلا ضبط للمتغير: لا تقييد (مسؤولية الناشر) | NFR-SEC-009، FR-ADM-002 |
...
> **تحديث 2026-09-13 — انحرافات تنفيذ موثقة (دون تغيير المستهدف):** (1) **PIN بscrypt لا Argon2id** — نفس انحراف PRODUCTION_READINESS A7 القائم (صلب عملياً، القرار: اعتماد رسمي أو ترحيل لاحق). (2) **الجلسة**: رموز خادمية عشوائية 48-hex بانتهاء 7 أيام على القناتين (كوكي + ترويسة x-sw-session) بدل JWT 15د + Refresh Rotation (ADR-005) — تدوير/كشف إعادة الاستخدام لم يُنفذا بعد. (3) **KMS**: APP_KEY من متغير بيئة/ملف دائم بدل مدير أسرار خارجي. (4) **Redis**: حدود المعدل بالذاكرة داخل العملية (كافية لعقدة واحدة — التوثيق نفسه في ADR-004).
...
## 8.3 تدفق الحدث الأمني (SecurityEvent — منفَّذ 2026-09-13)

```text
نقطة أمنية (login/verify فشل أو قفل أو نجاح — register — استرداد —
إعادة تعيين إدارية — رفض IP إداري)
        │
        ▼
logSecurityEvent(kind, { phone?, userId?, ip?, meta? })   [src/lib/server/security-events.ts]
        │  best-effort: فشل التسجيل يُسجل في console ولا يُفشل العملية الأمنية
        ▼
جدول SecurityEvent (kind, phone, userId, ip, metaJson, createdAt)
        │  فهارس: [createdAt] و[kind, createdAt]
        ▼
لوحة الإدارة: GET /api/admin/security-events?cursor&kind (ADMIN/COMPLIANCE)
        │  أحدثها أولاً + فلتر نوع + تسميات عربية
        ▼
إجراء الدعم: POST /api/admin/users/:id/reset-totp { reason }
        │  إبطال الجلسات + مسح TOTP والاسترداد + رمز تفعيل R+7 (24س)
        │  + رفع قفل الهاتف + Audit (TOTP_RESET) + إشعار المستخدم
        ▼
المستخدم يعيد الإلحاق: شاشة الدخول تقبل رمز R+7 → reenroll-confirm → TOTP جديد
```

> تصميم الحماية المرافق: قفل فشل **5 محاولات/15 دقيقة لكل هاتف** (AUTH-003) فوق حدود IP (30/ساعة — SYS-002)؛ و**منع إعادة استخدام رمز TOTP** عبر تخزين الخطوة الزمنية الناجحة (totpLastStep) على المستخدم؛ والأسرار كلها مشفرة بـAPP_KEY (خزنة AES-256-GCM — `crypto-vault.ts`).
